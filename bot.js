...
const trialEndedNotified = new Set();
...
async function fetchChannelMessages(channel, messageCount, timeframeMs) {
  ...
  const maxPages = Math.ceil(messageCount / 100) + 5;
  let allMessages = [];
  let lastId;
  let pages = 0;
  ...
  try {
    ...
    recentMsgs.forEach(m => {
      ...
      m.reactions.cache.forEach(r => {
        const count = r.count;
        total += count;
        reactionStr += `${r.emoji.name}x${count} `;
...
Respond ONLY with this exact JSON (no markdown, no extra text):
{
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.warn(`OpenAI attempt ${attempt} failed (${err.status}), retrying in ${delay}ms...`);
...
function buildScoreBar(score) {
  ...
  return 'X'.repeat(filled) + 'O'.repeat(10 - filled);
}
...
/**
 * Resolves a chart config to a Discord-safe image URL.
 *
 * Strategy:
 *  1. POST the config to QuickChart's /chart/create endpoint.
 *     This returns a short URL (e.g. https://quickchart.io/chart/render/9f...)
 *     that is always well under Discord's 2048-char embed image limit.
 *  2. If the POST fails for any reason, fall back to the GET URL.
 *     Only use the GET URL if it is <= 2048 chars; otherwise skip the chart
 *     entirely to avoid a DiscordAPIError that would crash the whole reply.
 *
 * @param {object} chartConfig  - Chart.js config object
 * @param {number} width        - Image width in pixels
 * @param {number} height       - Image height in pixels
 * @returns {Promise<string|null>} - Safe URL or null if the chart must be skipped
 */
async function getChartUrl(chartConfig, width, height) {
  // ── 1. Try POST API (short URL, always safe) ─────────────────────────────
  try {
    const res = await fetch('https://quickchart.io/chart/create', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ chart: chartConfig, width, height, backgroundColor: '#ffffff', version: 3 })
    });

    if (!res.ok) throw new Error(`QuickChart POST HTTP ${res.status}`);

    const data = await res.json();
    if (data?.url) {
      console.log(`Chart short URL: ${data.url.length} chars`);
      return data.url;
    }
    throw new Error('No URL in QuickChart POST response');
  } catch (postErr) {
    console.warn(`QuickChart POST failed (${postErr.message}), trying GET fallback`);
  }

  // ── 2. GET URL fallback ──────────────────────────────────────────────────
  const getUrl = `https://quickchart.io/chart?c=${encodeURIComponent(JSON.stringify(chartConfig))}&w=${width}&h=${height}&bkg=%23ffffff&version=3`;

  if (getUrl.length <= 2048) {
    console.log(`Chart GET URL: ${getUrl.length} chars`);
    return getUrl;
  }

  // URL too long -- skip rather than send a broken embed
  console.warn(`Chart URL too long (${getUrl.length} chars), skipping chart`);
  return null;
}

// ============================================================
// QUICKCHART GRAPH GENERATORS
// Each function returns { config, width, height } or null.
// Pass the result to getChartUrl() to get a safe Discord image URL.
// ============================================================
...
function buildReportEmbed(result, analyzedCount, channelNames, timeframeLabel, sensitivity, remaining, isPaidServer, channelReactions = {}, stats = null, isPublic = false, channelMsgCounts = {}, channelResults = {}, isTesterAccount = false) {
  ...
  if (isMulti) {
    ...
    sortedChs.forEach(ch => {
      ...
      const chEmoji = chScore >= 7 ? 'OK' : chScore >= 4 ? '~' : '!!';
  ...
  if (isPublic) {
    ...
    if (totalFlagged > 0) {
      ...
      if (flaggedValue.trim().length > 0) {
        embed.addFields({ name: '🚨 Most Severe Flagged Messages', value: flaggedValue, inline: false });
      }
    }
  }
  ...
  if (isMulti && !isPublic) {
    ...
    for (const ch of worstFirst) {
      ...
      if (reactionText) chValue += `\n💬 **Reactions**\n${reactionText}`;
      embed.addFields({ name: `🔍 ${ch}`, value: chValue.substring(0, 1024), inline: false });
    }
  }
  ...
  if (result.summary) embed.addFields({ name: '🗒️ Vibe Insights', value: result.summary.substring(0, 1024), inline: false });
  if (result.recommendation) embed.addFields({ name: '💡 AI Recommendations', value: result.recommendation.substring(0, 1024), inline: false });
...
async function registerCommands() {
  const vibeCommand = new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt.setName('sensitivity').setDescription('How strict the analysis is (default: medium)').setRequired(false).addChoices(
        ...
        { name: 'Medium -- General', value: 'medium' },
  ...
  const progressCommand = new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt.setName('sensitivity').setDescription('Filter by sensitivity level').setRequired(false).addChoices(
        { name: 'Low -- Gaming/Adult', value: 'low' },
        { name: 'Medium -- General', value: 'medium' },
        { name: 'High -- Kids/Family', value: 'high' }
      ))
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Medium', value: 'General (default)', inline: true },
...
async function handleVibeCommand(interaction) {
  ...
  if (!serverIsPaid && !tester && messageCount > CONFIG.FREE_MAX_MESSAGES) messageCount = CONFIG.FREE_MAX_MESSAGES;
  ...
  try {
    ...
    for (const ch of channels) {
      channelReactions[`#${ch.name}`] = await getChannelStats(interaction.guild, ch);
    }
    ...
    if (isPrivate) {
      try {
        ...
        await interaction.editReply('📬 Your private Vibe Report has been sent to your DMs!');
      } catch {
...
async function handleProgressCommand(interaction, range, filterChannels, sensitivity = null) {
  ...
  try {
    ...
    if (filterChannels && filterChannels.length > 0) {
      ...
      if (filteredReports.length === 0) {
        return interaction.editReply(`❌ No reports found for ${chNames.join(', ')}. Run \`/vibe\` in that channel first.`);
      }
      if (range !== '30d') {
        const baseLimit = parseInt(range) || 10;
        filteredReports = filteredReports.slice(-baseLimit);
      }
    }
    ...
    // Cap chart data at 50 most recent reports to keep payloads small
    const chartReports = filteredReports.slice(-50);

    // ── GENERATE CHART SPECS (sync) then resolve all URLs in parallel ─────────
    const scoreTrendSpec   = generateScoreTrendChart(chartReports);
    const sentimentSpec    = generateSentimentTrendChart(chartReports);
    const toxDoughnutSpec  = generateToxicityDoughnut(toxMap);
    const toxEvolutionSpec = generateToxicityEvolutionChart(chartReports);
    const channelRankSpec  = isMulti ? generateChannelRankingChart(channelStats) : null;

    // All five chart URL fetches run in parallel -- much faster than sequential
    const [scoreTrendUrl, sentimentUrl, toxDoughnutUrl, toxEvolutionUrl, channelRankUrl] = await Promise.all([
      scoreTrendSpec   ? getChartUrl(scoreTrendSpec.config,   scoreTrendSpec.width,   scoreTrendSpec.height)   : null,
      sentimentSpec    ? getChartUrl(sentimentSpec.config,    sentimentSpec.width,    sentimentSpec.height)    : null,
      toxDoughnutSpec  ? getChartUrl(toxDoughnutSpec.config,  toxDoughnutSpec.width,  toxDoughnutSpec.height)  : null,
      toxEvolutionSpec ? getChartUrl(toxEvolutionSpec.config, toxEvolutionSpec.width, toxEvolutionSpec.height) : null,
      channelRankSpec  ? getChartUrl(channelRankSpec.config,  channelRankSpec.width,  channelRankSpec.height)  : null
    ]);

    // ── EMBED 1: OVERALL SUMMARY + SCORE TREND ───────────────────────────────
    const embed1 = new EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          ...
          value:
            sentimentRow('Friendly',   '🟢', latest.friendly   || 0, newTotal) + `  _(was ${firstFPct}%${lastFPct-firstFPct !== 0 ? ` ${lastFPct-firstFPct > 0 ? 'up' : 'down'}${Math.abs(lastFPct-firstFPct)}%` : ''})_\n` +
            sentimentRow('Neutral',    '⚪', latest.neutral    || 0, newTotal) + '\n' +
            sentimentRow('Unfriendly', '🔴', latest.unfriendly || 0, newTotal) + `  _(was ${firstUPct}%${lastUPct-firstUPct !== 0 ? ` ${lastUPct-firstUPct > 0 ? 'up' : 'down'}${Math.abs(lastUPct-firstUPct)}%` : ''})_`,
          inline: false
        },
        {
    ...
    if (sentimentUrl && filteredReports.length >= 2) {
      ...
      const embed2 = new EmbedBuilder()
        ...
        .setDescription(
          `**Friendly:** ${firstFPct}% -> ${lastFPct}% ${fChange > 0 ? `🟢 up ${fChange}%` : fChange < 0 ? `🔴 down ${Math.abs(fChange)}%` : '-> unchanged'}\n` +
          `**Unfriendly:** ${firstUPct}% -> ${lastUPct}% ${uChange > 0 ? `🔴 up ${uChange}%` : uChange < 0 ? `🟢 down ${Math.abs(uChange)}%` : '-> unchanged'}`
        )
